...
const unreadCount = computed(() => notifications.value.filter(n => n.read === false).length)
...
<template>
  <header class="pt-header">
    ...
    <!-- Overlay / Panel ειδοποιήσεων -->
    <div v-if="applicationStore.isAuthenticated && notifOpen" class="notif-overlay" @click.self="closeNotifications">
      <aside class="notif-panel">
        <header class="notif-head">
          <h3>Notifications</h3>
          <div class="spacer"></div>
          <button class="mini ghost" :disabled="loading" @click="refreshNotifications">Refresh</button>
          <button class="mini" @click="closeNotifications">Close</button>
        </header>
...
.notif-overlay{
  ...
  display:flex; justify-content:flex-end; /* panel από δεξιά */
  z-index:80;
}
.notif-panel{
...
.notif-item.unread{ background:#f6f9ff; border-color:#d9e6fb; }
.row1{ display:flex; align-items:center; justify-content:space-between; gap:12px; }
.type{
  font-size:12px; font-weight:800; color:#0b2e55; background:#e9f0fb; padding:2px 8px; border-radius:999px;
}

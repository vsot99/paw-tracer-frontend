...
const urlRef = ref(`${backend}/api/lost-pet-reports/my-reports`)
...
<template>
  <section>
    ...
    <!-- 2 κάρτες ανά σειρά, full width section -->
    <div v-else-if="items.length" class="cards-grid">
      <ReportRow v-for="it in items" :key="it.id" :item="it" type="lost" />
    </div>

    <p v-else class="empty">You haven’t submitted any lost pet reports yet.</p>
  </section>
</template>

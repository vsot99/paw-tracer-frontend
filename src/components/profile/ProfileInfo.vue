...
async function loadUser() {
  ...
  try {
    ...
    errorMsg.value = e.message || 'Αποτυχία φόρτωσης στοιχείων.'
  } finally {
    loading.value = false
  }
}
...
function initMap() {
  ...
  const center = {
    lat: Number(form.latitude ?? 37.9838),
    lng: Number(form.longitude ?? 23.7275),
  }
...
/* Edit */
.edit-grid { display:grid; grid-template-columns: 3fr 2fr; gap:16px; align-items:start; }
.col { min-width:0; }
.fields { display:grid; gap:12px; }
.field { display:grid; gap:6px; }
.field > span { color:#0b2e55; font-weight:700; font-size:14px; }
input { height:46px; padding:0 12px; border-radius:12px; border:1px solid rgba(0,0,0,.14); font-size:16px; outline:none; }
